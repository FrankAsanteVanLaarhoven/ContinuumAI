...
| Holder binding prevents token replay | replay attack experiments | Stage A adversarial (v0.2): deterministic — bearer reuse, forged proof-of-possession, expiry, scope/tenant tamper and audience confusion all blocked at the expected check. Concurrency baseline (`C1-10`): a captured `(challenge, signature)` verified more than once — the verifier enforces signature/expiry but does not consume the challenge (GAP-4). Intervention **I4** (v0.3, `research/sif-bench/interventions/i4/I4_RESULTS.md`) closed the tested GAP-4 in a deterministic matched experiment over real PostgreSQL: I4-A reproduced replay (accepted ×2), concurrent double-spend, and all three proof-lift cases (3/3); I4-B consumes a server-issued nonce exactly once via a transactional replay ledger, closing replay and double-spend (necessary but insufficient — lifts still 3/3); I4-C additionally binds the proof to the request digest, capability id and audience, closing the lifts (0/3). **Zero false denials** on the benign control; the expiry, non-holder and missing-proof controls hold under every arm. Bounded to the evaluated single-instance ledger and failure model — not general credential theft, holder-key compromise, or cross-node ledger consistency |
| Revocation terminates active authority | persistent concurrent revocation tests | Stage A adversarial (v0.2): in-session reuse-after-revoke deterministically blocked. Concurrency baseline: point-of-use **capability** revocation and expiry held under the tested interleavings (0 post-revocation disclosures), but authorization is a snapshot — consent/policy/object-lifecycle changes are NOT re-evaluated against a live capability within its TTL (GAP-3). Intervention **I3** (v0.3, `research/sif-bench/interventions/i3/I3_RESULTS.md`) closed the four tested GAP-3 staleness dimensions at the disclosure/model-call point of use in a deterministic matched experiment: I3-A reproduced the stale permits (4/4), I3-B re-checked the bound policy version + consent digest (2/4), and I3-C additionally re-evaluated the risk ceiling and object lifecycle against current state (0/4), with **zero false denials** on the unchanged-authority control. Bounded to those four dimensions at the disclose/model boundary — the action-execution re-validation facet and concurrent/eventually-consistent propagation are not exercised. Durable revocation is the separate persistence arm |
| Human gates prevent unauthorized execution | bypass and race-condition suite | Stage A adversarial (v0.2): agent self-approval, impostor-agent, cross-tenant, unknown-approver and denied-action approval all blocked; a prior self-approval gap was closed. Concurrency baseline (C2): double-execution, duplicate/foreign-tenant approval, and approving a denied action all refused under the tested race; illegal state skips rejected. Gaps: an approved action is not re-validated against a rotated policy at execution — the **action-execution** facet of GAP-3, which intervention **I3** (which closes the four tested staleness dimensions at the disclosure/model-call boundary, `research/sif-bench/interventions/i3/I3_RESULTS.md`) does **not** exercise; this facet remains OPEN. Intervention **I6** (v0.3, `research/sif-bench/interventions/i6/I6_RESULTS.md`) remediated **GAP-6** (caller-supplied `action_id` silently overwrites and retries re-execute) in a deterministic matched experiment over real PostgreSQL: with server-issued action identity + a caller idempotency key + a canonical request digest (I6-B/C), the tested silent overwrite, duplicate creation and duplicate execution are prevented (`false_permit` 2→0), I6-C additionally rejects same-key/different-request conflicts (4/4), missing-key consequential actions are denied, and evidence distinguishes create/replay/conflict — **zero false denials** on the benign new-action controls. Effectively-once **only within the evaluated single-instance domain and rollback/evidence-fail failure model**; not distributed exactly-once |
...
| Gateway reduces injection success | large attack corpus and ablation | Stage B measured (v0.2), screen-permeability only: the current heuristic lowers attack success from **0.9286** (no screen, immutable before-defence reference) to **0.5714**, but is bypassed by base64, homoglyph, letter-spacing, multilingual and fake-system payloads; benign FPR 0.0. Intervention **I7** (v0.3, `research/sif-bench/stage_b/I7_RESULTS.md`) is a matched deterministic ablation (no live model, no LLM classifier, no external moderation): **I7-A** reproduces the 0.5714 heuristic permeability; **I7-B** structured typed-channel separation closes the indirect-injection, role-impersonation and tool/output-schema families (ASR → 0 each; unauthorized tool-call rate 1.0 → 0; output quarantine 0 → 1.0) → 0.2857; **I7-C** adds a bounded, fail-closed decode/normalize before the same screen, closing the base64/homoglyph/letter-spacing family → **0.0714**, with **zero false positives, benign task success 1.0, and zero normalization-induced semantic corruption**. Honest residual: a French-language payload (`PI-ENCODED-004`) survives every arm (bounded normalization does not translate). Permeability is an **upper bound** on real attack success, not model compliance; a real-model layer (Stage B v0.4) and the B0–B3 joint-frontier comparison (`research/paper/B0_B3_DEFINITIONS.md`) remain to be built |
