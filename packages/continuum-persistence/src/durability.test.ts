...
import { serviceRef, withServiceCtx } from "../test/identity";
...
describe("durability and restart-safe evidence", () => {
  it("re-verifies the persisted hash chain over a fresh connection", async () => {
    const result = await verifyPersistedChain(pool, serviceRef("t_acme"));
  ...
  it("persists an evidence envelope for every material step", async () => {
    ...
    const persisted = await countRows(pool, serviceRef("t_acme"), "evidence_envelopes");
    expect(persisted).toBe(inMemory);
  });

  it("keeps evidence ordered and gap-free", async () => {
    const entries = await loadEvidence(pool, serviceRef("t_acme"));
  ...
  it("persists the capability and its revocation durably", async () => {
    const caps = await countRows(pool, serviceRef("t_acme"), "capabilities");
    const revs = await countRows(pool, serviceRef("t_acme"), "revocations");
    expect(caps).toBe(1);
    expect(revs).toBe(1);
  });

  it("persists the action state machine outcomes", async () => {
    const states = await withServiceCtx(pool, "t_acme", async (c) =>
    ...
    const approvals = await countRows(pool, serviceRef("t_acme"), "approvals");
  ...
  it("reproduces migrations from an empty database (idempotent re-run)", async () => {
    ...
    // policies is a global (non-RLS) table; readable without tenant context.
    const policies = await withServiceCtx(pool, "t_acme", async (c) =>
